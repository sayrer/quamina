...
type vmFields struct {
	...
	shellFastMatchers   []*shellFastMatcher // fast path for pure shell-style patterns (single pattern)
	hasNumbers          bool
...
func (m *valueMatcher) transitionOn(eventField *Field, bufs *nfaBuffers) []*fieldMatcher {
	...
	// Fast path: check shell fast matchers first (SIMD-optimized bytes.Index).
	// val includes surrounding quotes; strip them before matching.
...
func (m *valueMatcher) addTransition(val typedVal, printer printer, bufs *closureBuffers, buildMode MatcherBuildMode) *fieldMatcher {
	...
	case shellStyleType:
		// Fast-path: if this is the very first shellstyle pattern and no NFA or singleton
		// exists yet, create a shellFastMatcher instead of building an NFA.
		if fields.start == nil && fields.singletonMatch == nil && len(fields.shellFastMatchers) == 0 {
		...
		// Migrate any existing fast matchers to NFA before adding this new pattern.
		if len(fields.shellFastMatchers) > 0 {
			for _, sfm := range fields.shellFastMatchers {
				...
				if fields.start == nil {
					...
					fields.start = mergeStartStates(fields.start, fa, printer)
				}
			}
		...
		// Add this new shellstyle pattern to the NFA.
